...
<script>
    ...
    export default {
        ...
        methods: {
            makeReservation() {
                if (this.isFormValid) {
                    let endDate = new Date(this.form.date);
                    endDate.setDate(endDate.getDate() + parseInt(this.form.numberOfDays));
                    axios.post('https://isa-projekat-tim-3.herokuapp.com/reservations/make-for-client/' + this.$route.params.id, {
        ...
        mounted() {
            ...
            axios.get('https://isa-projekat-tim-3.herokuapp.com/' + path + '/get/' + this.$route.params.id, {
                headers: {
                    Authorization: 'Bearer ' + window.sessionStorage.getItem('accessToken')
                }
            }).then((response) => {
